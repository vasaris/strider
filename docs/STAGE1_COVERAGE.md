...
| scene_details (solo) | **ENGINE** | journey катит `scene_details` через `listByType` с Stage 1 (cite `journey/scene.ts:42-47`); сюрфейсинг детали в пакет — **SD1@2.4** |
...
- **Аудит выявил новое** (не было ни в одном плане): **TR1** (сокровища),
  **PT1** (задачи покровителей). Оба поздне-этапные, ни один не блокирует
  Stage 2; занесены в реестр.
- **Корректировка (2026-06-26):** строка `scene_details` / **SD1** была **ложным**
  deferred-вердиктом. Движок катит `scene_details` в journey с Stage 1
  (`journey/config.ts:221` `listByType`, `journey/scene.ts:42-47` `rollSuccessDie`
  + `significantEncounter`; покрыто golden `dark-1`). Открытый остаток сужен до
  сюрфейсинга детали в пакет хода (**SD1@2.4**, orchestrator).
- **Методологическая сноска.** Детекция потребления шла грепом по id-литералам —
  он **не ловит** bulk-загрузку через `listByType`. Перепроверено: bulk-типов
  два — `scene_detail_table` и `rule_card`. `scene_detail_table` потреблён (SD1 —
  единственный ложный вердикт); `rule_card` сканируется прогрессией только ради
  advancement-полей. **HC1 / FV1 / TR1 / PT1** подтверждены (держатся).
