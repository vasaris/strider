...
5. **Выход Stage 0:** `python3 tools/extraction/mark_verified.py --dir content-packs/kv/mechanics --id-prefix kv. --gate2 "<2a:GATE2A_KV_FINDINGS.md @<commit> + F1/F2/F3; 2b:gate2b_evidence/MANIFEST.md>" --gate3 "<lynn evidence>"` — штампует 140 КВ (49 ИдО idempotent-skip, если уже verified). Если ИдО ещё не стэмпнуты — без `--id-prefix` штампует все 189 разом (убедись, что ИдО прошли свой 2a/3 — см. `GATE2A_IDO_FINDINGS.md` + `HANDOFF_IDO_S1_BACKFILL.md`).
   *(Префикс-фильтр `--id-prefix` добавлен в `mark_verified.py` в рамках ИдО-выхода — TASK B из `HANDOFF_IDO_S1_BACKFILL.md`; для КВ используется `--id-prefix kv.` либо запуск без фильтра.)*
